    ...
    switch (opcode & 0xF000) {
        ...
        case 0x8000:
            ...
            switch (opcode & 0xF00F) {
                ...
                case 0x8005: {
                    // printf("0x8__5\n");
                    uint8_t tmp_x = chip8->registers[x];
                    chip8->registers[x] -= chip8->registers[y];
                    chip8->registers[0xF] = tmp_x >= chip8->registers[y] ? 1 : 0;
                    break;
                }
                case 0x8006:
                    // printf("0x8__6\n");
                    if (chip8->legacy_version) {
                        // Old Systems
                        uint8_t carry = chip8->registers[y] & 0x0001;
                        chip8->registers[x] = chip8->registers[y] >> 1;
                        chip8->registers[0xF] = carry;

                    } else {
                        // Newer Systems
                        uint8_t carry = chip8->registers[x] & 0x0001;
                        chip8->registers[x] >>= 1;
                        chip8->registers[0xF] = carry;
                    }
                    break;
                ...
                case 0x8007: {
                    ...
                    chip8->registers[0xF] = chip8->registers[y] >= tmp_x ? 1 : 0;
                    break;
                }
                ...
                case 0x800E:
                    ...
                    if (chip8->legacy_version) {
                        // Old Systems
                        uint8_t carry = (chip8->registers[y] & 0x80) >> 7;
                        chip8->registers[x] = chip8->registers[y] << 1;
                        chip8->registers[0xF] = carry;

                    } else {
                        // Newer Systems
                        uint8_t carry = (chip8->registers[x] & 0x80) >> 7;
                        chip8->registers[x] <<=  1;
                        chip8->registers[0xF] = carry;
                    }
                    break;
        ...
        case 0xF000:
            switch (opcode & 0xF0FF) {
                ...
                case 0xF00A:
                    if (!chip8->waiting_for_key_release) {
                        for (int i = 0; i < 16; i++) {
                            if (chip8->keypad[i]) {
                                chip8->key_to_release = i;
                        ...
                        // If key released
                        if (!chip8->keypad[chip8->key_to_release]) {
                            chip8->registers[x] = (uint8_t)chip8->key_to_release;
